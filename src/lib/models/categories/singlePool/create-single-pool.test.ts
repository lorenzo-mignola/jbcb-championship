...
describe('singlePool odds', () => {
  ...
  it('should not have duplicate', () => {
    ...
    const onlyId = matches
      .map(({ blue, white }) => {
        const sorted = [white?.id, blue?.id].filter((a): a is string =>
          Boolean(a),
        );
        return sorted;
      })
...
describe('singlePool even', () => {
  ...
  it.each([[4], [5], [6]])(
    'all athletes do the same numbers of match',
    (slice) => {
      const pool = createSinglePool(name, athletes.slice(0, slice), 0);

      const nrOfMatch = pool.matches.reduce<Record<string, number>>(
        (counter, match) => {
  ...
  it('should not have duplicate', () => {
    const pool = createSinglePool(name, athletes, 0);
    const { matches } = pool;
    const onlyId = matches.map(({ blue, white }) => {
      if (white?.id === blue?.id) {
        expect.fail();
      }
