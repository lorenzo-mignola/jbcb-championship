...
function isPoolMatch(category: DoublePoolCategory, matchId: string) {
  const { A, B } = category.pools;
  return (
    A.some(match => match.id === matchId)
    || B.some(match => match.id === matchId)
  );
}

function isPoolAorB(category: DoublePoolCategory, matchId: string) {
  const { A, B } = category.pools;
...
function isSemiFinalMatch(category: DoublePoolCategory, matchId: string) {
  const { semifinals } = category;
  return semifinals.some(match => match.id === matchId);
}

function isFinalMatch(category: DoublePoolCategory, matchId: string) {
  const { finalMatch } = category;
  return finalMatch.id === matchId;
}

function getWhiteAndBlueSemifinal(category: DoublePoolCategory, index: 0 | 1) {
  const winnerPool = category.pools[index === 0 ? 'A' : 'B'];
  const winnerPoolAthletes
    = category.pools[index === 0 ? 'aAthletes' : 'bAthletes'];
  const loserPool = category.pools[index === 0 ? 'B' : 'A'];
  const loserPoolAthletes
    = category.pools[index === 0 ? 'bAthletes' : 'aAthletes'];
  const whiteId = getRankingPool(winnerPool, winnerPoolAthletes).find(
    rank => rank.rank === 1,
  )?.id;
  const blueId = getRankingPool(loserPool, loserPoolAthletes).find(
    rank => rank.rank === 2,
  )?.id;
  return {
    blue: resetAthlete(
      category.athletes.find(athlete => athlete.id === blueId),
    ),
    white: resetAthlete(
      category.athletes.find(athlete => athlete.id === whiteId),
    ),
  };
}
...
function updatePoolMatch(category: DoublePoolCategory, matchUpdated: Match) {
  const currentMatch = category.matches.findIndex(
  ...
  const poolUpdated = produce(category.pools, (pools) => {
    pools[isPoolAorB(category, matchUpdated.id)] = pools[
      isPoolAorB(category, matchUpdated.id)
    ].map((match) => {
...
function updateSemiFinalMatch(
  ...
  if (isLastSemifinal) {
    const winnerFirstSemi
      = categoryUpdated.semifinals[0][categoryUpdated.semifinals[0].winner!];
    const winnerSecondSemi
      = categoryUpdated.semifinals[1][categoryUpdated.semifinals[1].winner!];
...
export function updateDoublePool(
  category: DoublePoolCategory,
  matchUpdated: Match,
): DoublePoolCategory {
