...
describe('createBrackets', () => {
  ...
  it.skip.each([
    ...
    const lastMatch = brackets.rounds[0].winner[brackets.rounds[0].winner.length - 1];
    ...
    expect(
      bracketsUpdated.rounds[1].winner[bracketsUpdated.rounds[1].winner.length - 1].white!.id
    ).toBe(white!.id);
  });
});
